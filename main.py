...
def fetch_reddit(limit=20):
    ...
    for sub in subreddits:
        try:
            resp = requests.get(f'https://www.reddit.com/r/{sub}/hot.json?limit={limit}',
...
'#FF4500', 'VK': '#0077FF', 'HoYoLAB': '#1A9DD9'}
...
def section(title, posts, icon):
    if not posts: return f'<section><h2>{icon} {title}</h2><p class="empty">No data this week.</p></section>'
...
main{{max-width:1100px;margin:0

auto;padding:2rem 1rem}}section{{margin-bottom:3rem}}
